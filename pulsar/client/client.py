...
PULSAR_CONTAINER_IMAGE = "ghcr.io/kysrpex/galaxyproject-pulsar:arc"
...
class ARCLaunchMixin(BaseRemoteConfiguredJobClient):
    ...
    def launch(
        ...
        tool_container = CoexecutionContainerCommand(
            ...
            "bash",
            ["-x", f"{job_config['job_directory']}/command.sh"],
            job_config["job_directory"],
            guest_ports,
        )
    ...
    def _generate_executable(
        ...
        return dedent(f"""
            ...
            singularity run \\
                ...
                https://github.com/kysrpex/galaxyproject-pulsar/releases/download/5c6d506/galaxyproject-pulsar@5c6d5067.sif \\
            ...
            cat job_directory/{output_manifest_path} | jq -r '.[] | "file://\(.from_path | sub("^{staging_directory}/"; "job_directory/")) \(.url)"' > output.files            
        """).encode("utf-8")
    ...
    def _generate_job_description(
        ...
        for input_ in input_manifest:
            ...
            uri.text = input_["url"]  #.replace("https://galaxy-3923.local", "http://bc8f8927628077.lhr.life")
    ...
    def kill(self):
        # take form the original pull request
        pass
